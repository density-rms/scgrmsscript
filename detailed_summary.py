...
def apStatsSum(newdate,summaryDict):
    ...
    if (os.stat("%s" % fname).st_size == 0):
        ...
        sys.exit(1)
    for line in fp:
        lastfile = line.split()[-1]
        if lastfile.startswith('ap_stats_'):
            break
    fp.close()

    dDict = {}
    if lastfile:
        df = pd.read_csv('%s/%s' % (apstats_dir,lastfile))
        #2g_ch_util_Busy,2g_ch_util_RX,2g_ch_util_TX,2g_ch_util_Total
        g_Busy_sum = df['2g_ch_util_Busy'].sum() 
        g_TX_sum = df['2g_ch_util_TX'].sum() 
        g_RX_sum = df['2g_ch_util_RX'].sum() 
        g_Total_sum = df['2g_ch_util_Total'].sum() 
        a_Busy_sum = df['5g_ch_util_Busy'].sum() 
        a_TX_sum = df['5g_ch_util_TX'].sum() 
        a_RX_sum = df['5g_ch_util_RX'].sum() 
        a_Total_sum = df['5g_ch_util_Total'].sum()
        g_Busy_mean = df['2g_ch_util_Busy'].mean() 
        g_TX_mean = df['2g_ch_util_TX'].mean() 
        g_RX_mean = df['2g_ch_util_RX'].mean() 
        g_Total_mean = df['2g_ch_util_Total'].mean() 
        a_Busy_mean = df['5g_ch_util_Busy'].mean() 
        a_TX_mean = df['5g_ch_util_TX'].mean() 
        a_RX_mean = df['5g_ch_util_RX'].mean() 
        a_Total_mean = df['5g_ch_util_Total'].mean()        
    print g_Busy_sum,g_RX_sum,g_TX_sum,g_Total_sum,a_Busy_sum,a_RX_sum,a_TX_sum,g_Total_sum
    print
    print g_Busy_mean,g_RX_mean,g_TX_mean,g_Total_mean,a_Busy_mean,a_RX_mean,a_TX_mean,a_Total_mean
...
if __name__ =='__main__':      
    ...
    #summaryDict['Video54 SZ100 Image'] = '3.2.2.2' 
    #apStatsSum(new_date, summaryDict)
    ...
    fname = '/var/www/video54status/dailystatus/rsm_summary.csv'
    subprocess.call(['%s/rsm_stats_email.sh' % working_dir,  "%s" % fname])
