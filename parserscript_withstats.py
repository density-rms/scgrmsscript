...
def rsm_writer(statsDict,tstamp):
    csvfile1 = 'rsmdata_%s.csv' % tstamp 
    csvPtr1 = open(csvfile1,"w") 
    fieldnames1 = ['ap','ap_ip','proc','count']
    dwriter1= csv.DictWriter(csvPtr1,fieldnames1, dialect = 'excel', delimiter= ',', extrasaction='ignore')
    csvPtr1.write(','.join(fieldnames1)+'\r\n')
    for ap in statsDict.keys():
        for proc in sorted(statsDict[ap].keys()):
            #print statsDict[ap][wlan]
            dwriter1.writerow(statsDict[ap][proc])
    csvPtr1.close()
...
def rsm_summary_writer(newprocDict,dDict,newDict,tstamp):
    ...
    csvfile1 = "rsm_summary_%s.csv" % tstamp
    ...
    fieldnames = ['ap','List of restarted process and count)']
...
def utc_localtime_conversion(ts,todaydate):
    (m,d,t) = ts.split()
    if m in months:
        m_index = months.index(m) 
    utc_str ='-'.join(['2015',str(m_index+1),d,t])  
    #print utc_str
...
def ap_reboot_dict(timestamp,email_flag,apDict,ap_clientDict,apmacDict,apipDict,apmodelDict,message_fp,heartbeat_ap_list):
    ...
    c_dir = os.getcwd() 
    print c_dir
    os.system('cat %s/writable/event.log | grep %s > eventlog' % (c_dir,yesterdaydate))
    os.system('cat %s/writable/event.log | grep %s >> eventlog' % (c_dir,todaydate))
    ...
    #os.system('cat %s/eventlog |grep -i "apRebootByUser" > apreboot' % (c_dir))
    ...
    #pattern27 = "([0-9T:\s\-]+).*?apRebootBySystem,apMac=(.*?),reason=([\w\s]+),"
...
def file_parser(timestamp,email_flag):
    ...
    message_fp.write('The following events happened on SZ100 NETWORK in the last half hour\n\n')
    apDict = {} 
        ...
        if res81:
            ...
            for item in res81:
                if int(item[1]) > 0:
                    ...
                    statsDict[ap][proc]['ap'] = ap
                    statsDict[ap][proc]['ap_ip'] = ip
    ...
    csv3 = rsm_summary_writer(newprocDict,dDict,newDict)
...
#################################################################
    for f in datafilelist:
        ...
        if res11:
            ...
            ts_new = utc_localtime_conversion(adate,todaydate)
            if ts_new:
                (m,d,t) = ts_new.split()
                slist = [int(x) for x in t.split(':')]
...
##Tx Descr stuck
    ...
    for f in logfilelist:
        ...
        if res7:
            for t in res7:
                ts_utc = t[0]
                ts = utc_localtime_conversion(ts_utc,todaydate)
                if ts:              
                    wlan = t[1]
                    station = t[2]
                    if station:
